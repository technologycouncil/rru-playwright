...
\f0\fs24 \cf0 // rru-kaltura-youtube-crawler-batch.js\
//\
// Install:\
...
\
if (!INPUT) \{\
  console.error('Usage: node rru-kaltura-youtube-crawler-batch.js <course-url-or-csv-file> [--headed] [--output "/path/to/output"] [--max-pages 2000]');\
  process.exit(1);\
\}\
\
...
\}\
\
function ensureCourseDirPath(dirPath) \{\
...
\}\
\
function activityFolderFromPage(pageTitle, pageUrl) \{\
  return activityFolderBaseFromPage(pageTitle, pageUrl);\
\}\
\
...
\}\
\
function withPlayerSkin(url, playerSkinId = CONFIG.playerSkinId) \{\
  if (!url) return '';\
\
  let output = url;\
\
  // Moodle/KAF embed style:\
  // playerSkin/23449618 or playerSkin%2F23449618\
  const playerSkinRegex = /(playerSkin(?:%2F|\\/))\\d+/i;\
  if (playerSkinRegex.test(output)) \{\
    output = output.replace(playerSkinRegex, `$1$\{playerSkinId\}`);\
  \}\
\
  // Direct Kaltura embed style:\
  // /uiconf_id/23449615/\
  const uiconfPathRegex = /(\\/uiconf_id\\/)\\d+/i;\
  if (uiconfPathRegex.test(output)) \{\
    output = output.replace(uiconfPathRegex, `$1$\{playerSkinId\}`);\
  \}\
\
  // Query param style:\
  // ?uiconf_id=23449615\
...
\
    if (parsed.searchParams.has('uiconf_id')) \{\
      parsed.searchParams.set('uiconf_id', playerSkinId);\
      output = parsed.href;\
    \}\
  \} catch \{\}\
\
...
\
  return (\
    ...
    url.includes('api.cap2.ovp.kaltura.com') ||\
...
\
    ...
    \}\
\
    // Important: do NOT skip forceview anymore.\
    // Moodle's Jump to... menu uses forceview=1 for many valid activities.\
\
...
\
        ...
        await loc.type(value, \{ delay: 15, timeout: 10000 \});\
\
...
\
  try \{\
    const submitted = await passwordLocator.evaluate(el => \{\
...
\
    if (submitted) \{\
      console.log('Submitted login form via password field parent form.');\
      await page.waitForLoadState('domcontentloaded', \{ timeout: 30000 \}).catch(() => \{\});\
      await page.waitForTimeout(3000);\
    \}\
  \} catch \{\
    console.log('Could not submit parent form directly. Trying buttons...');\
  \}\
\
  const stillHasPassword = await page.locator('input[type="password"]').count().catch(() => 0);\
\
  if (stillHasPassword > 0 || /login|signin|sign-in|saml|cas|auth|adfs/i.test(page.url())) \{\
...
\
    ...
    \}\
\
    // Moodle activity dropdown / "Jump to..." select often contains the full course sequence.\
...
\
  const inspectUrl = url => \{\
    const found = extractYoutubeFromText(url);\
    if (found && !youtubeUrl) youtubeUrl = found;\
  \};\
\
  probePage.on('request', request => inspectUrl(request.url()));\
  probePage.on('response', response => inspectUrl(response.url()));\
\
  probePage.on('console', message => \{\
    const found = extractYoutubeFromText(message.text());\
    if (found && !youtubeUrl) youtubeUrl = found;\
  \});\
\
  try \{\
...
\
            if (data.event === 'command' && data.args && data.args[0]) \{\
...
\
    youtubeUrl = youtubeUrl || extractYoutubeFromText(await probePage.content());\
\
...
\
      try \{\
        youtubeUrl = youtubeUrl || extractYoutubeFromText(await frame.content());\
      \} catch \{\}\
    \}\
\
    await probePage.waitForTimeout(1500);\
...
\
        const ks =\
          ...
          '';\
\
        return \{ entryId, partnerId, uiConfId, serviceUrl, cdnUrl, ks \};\
      \});\
\
      if (data.entryId && data.partnerId && data.serviceUrl) \{\
        return data;\
      \}\
    \} catch \{\}\
  \}\
...
\
  const url =\
    `$\{config.serviceUrl.replace(/\\/$/, '')\}/service/flavorasset/action/getByEntryId` +\
    `?format=1` +\
    ksPart +\
...
\
  if (!response.ok()) \{\
    throw new Error(`Kaltura flavorAsset.getByEntryId failed: HTTP $\{response.status()\}`);\
  \}\
\
...
\
  const url =\
    `$\{config.serviceUrl.replace(/\\/$/, '')\}/service/flavorasset/action/$\{action\}` +\
    `?format=1` +\
    ksPart +\
...
\
  if (!response.ok()) \{\
    throw new Error(`Kaltura flavorAsset.$\{action\} failed: HTTP $\{response.status()\}`);\
  \}\
\
...
\
    const readyFlavors = flavors\
      ...
      .sort((a, b) => \{\
        ...
        return bHeight - aHeight || bBitrate - aBitrate || bSize - aSize;\
      \});\
\
...
\
      for (const getUrl of candidateActions) \{\
        try \{\
...
\}\
\
async function clickDownloadTextEverywhere(page) \{\
  try \{\
    const loc = page.getByText(/download/i).first();\
\
    if (await loc.count()) \{\
...
\
  for (const frame of page.frames()) \{\
    try \{\
      const loc = frame.getByText(/download/i).first();\
\
      if (await loc.count()) \{\
        await loc.click(\{ timeout: 2500, force: true \});\
        return true;\
...
\}\
\
async function clickSelectorsEverywhere(page, selectors, timeout = 1500, which = 'first') \{\
  for (const selector of selectors) \{\
    try \{\
      const loc = which === 'last'\
...
\}\
\
async function getLikelyPlayerBoxes(page) \{\
  const boxes = [];\
\
  async function addBox(locator) \{\
    try \{\
...
\
  ...
  \}\
\
  await addBox(page.locator('video').first());\
  await addBox(page.locator('.playkit-player').first());\
  await addBox(page.locator('[class*="player" i]').first());\
  await addBox(page.locator('body').first());\
\
  for (const frame of page.frames()) \{\
    try \{\
      await addBox(frame.locator('video').first());\
      await addBox(frame.locator('.playkit-player').first());\
      await addBox(frame.locator('[class*="player" i]').first());\
      await addBox(frame.locator('body').first());\
    \} catch \{\}\
  \}\
\
  return boxes;\
\}\
\
async function clickKalturaDownloadControl(page) \{\
...
\
  if (await clickSelectorsEverywhere(page, directSelectors)) return true;\
  if (await clickDownloadTextEverywhere(page)) return true;\
\
  if (await clickSelectorsEverywhere(page, menuSelectors)) \{\
    await page.waitForTimeout(700);\
\
    if (await clickSelectorsEverywhere(page, directSelectors, 2500)) return true;\
    if (await clickDownloadTextEverywhere(page)) return true;\
  \}\
\
  const boxes = await getLikelyPlayerBoxes(page);\
\
  for (const box of boxes) \{\
    const candidates = [\
      \{ x: box.x + box.width - 18, y: box.y + 18 \},\
      \{ x: box.x + box.width - 24, y: box.y + 24 \},\
      \{ x: box.x + box.width - 32, y: box.y + 24 \},\
      \{ x: box.x + box.width - 42, y: box.y + 24 \},\
      \{ x: box.x + box.width - 54, y: box.y + 24 \},\
      \{ x: box.x + box.width - 66, y: box.y + 24 \},\
      \{ x: box.x + box.width - 80, y: box.y + 24 \},\
      \{ x: box.x + box.width - 96, y: box.y + 24 \},\
      \{ x: box.x + box.width - 116, y: box.y + 24 \},\
      \{ x: box.x + box.width - 136, y: box.y + 24 \},\
    ];\
\
    for (const point of candidates) \{\
      try \{\
        ...
        await page.waitForTimeout(700);\
\
        if (await clickSelectorsEverywhere(page, directSelectors, 1000)) return true;\
        if (await clickDownloadTextEverywhere(page)) return true;\
\
        return true;\
      \} catch \{\}\
...
\}\
\
async function waitForDownloadModal(page) \{\
  const checks = [\
    async () => page.getByText(/^Downloads$/i).first(),\
    async () => page.getByText(/Main stream/i).first(),\
    async () => page.getByText(/Select quality/i).first(),\
    async () => page.getByText(/Source\\s*\\(/i).first(),\
  ];\
\
  for (const makeLocator of checks) \{\
    try \{\
      const loc = await makeLocator();\
      await loc.waitFor(\{ timeout: 2500 \});\
      return true;\
    \} catch \{\}\
  \}\
\
  for (const frame of page.frames()) \{\
    for (const pattern of [/^Downloads$/i, /Main stream/i, /Select quality/i, /Source\\s*\\(/i]) \{\
      try \{\
        await frame.getByText(pattern).first().waitFor(\{ timeout: 800 \});\
        return true;\
      \} catch \{\}\
    \}\
  \}\
...
\}\
\
async function clickSourceRowByDom(page) \{\
  const patterns = [\
    /Source\\s*\\(\\d+p\\)/i,\
    /Source\\s*\\(/i,\
    /\\.(mov|mp4|m4v|avi|wmv|webm)/i,\
  ];\
\
  for (const frame of page.frames()) \{\
    for (const pattern of patterns) \{\
...
\}\
\
async function getDownloadModalRowClickPoints(page) \{\
  const points = [];\
  const offsets = [180, 160, 200, 140, 220, 120, 240, 100, 260, 80, 300, 60, 340];\
\
  const textPatterns = [\
    /Source\\s*\\(\\d+p\\)/i,\
    /Source\\s*\\(/i,\
...
\
        for (const offset of offsets) \{\
          points.push(\{\
            ...
            y,\
            label: `frame right of source text +$\{offset\}`,\
          \});\
        \}\
      \} catch \{\}\
    \}\
...
\
      for (const offset of offsets) \{\
        points.push(\{\
          x: box.x + box.width + offset,\
          y,\
          label: `page right of source text +$\{offset\}`,\
        \});\
      \}\
    \} catch \{\}\
  \}\
\
  const ys = [153, 145, 160, 168, 136];\
\
  for (const y of ys) \{\
    points.push(\
      \{ x: 500, y, label: 'fixed arrow/right row' \},\
      \{ x: 485, y, label: 'fixed arrow/right row' \},\
      \{ x: 470, y, label: 'fixed right row' \},\
      \{ x: 450, y, label: 'fixed right row' \},\
      \{ x: 430, y, label: 'fixed right row' \},\
      \{ x: 400, y, label: 'fixed middle-right row' \},\
      \{ x: 380, y, label: 'fixed middle row' \},\
      \{ x: 340, y, label: 'fixed middle row' \},\
      \{ x: 320, y, label: 'fixed middle row' \},\
      \{ x: 270, y, label: 'fixed title row' \},\
      \{ x: 220, y, label: 'fixed title row' \},\
      \{ x: 170, y, label: 'fixed title row' \},\
      \{ x: 130, y, label: 'fixed left-middle row' \}\
    );\
  \}\
\
  return points;\
...
\
  ...
  probePage.setDefaultNavigationTimeout(45000);\
\
  const skinnedUrl = withPlayerSkin(iframeUrl);\
\
...
\
  try \{\
    await probePage.goto(skinnedUrl, \{ waitUntil: 'domcontentloaded', timeout: 45000 \});\
...
\
    ...
    \}\
\
    await probePage.mouse.move(1200, 80).catch(() => \{\});\
    await probePage.waitForTimeout(700);\
\
    const firstDownloadPromise = probePage\
      .waitForEvent('download', \{ timeout: 5000 \})\
      .catch(() => null);\
\
    const clickedMainDownload = await clickKalturaDownloadControl(probePage);\
\
    if (!clickedMainDownload) \{\
      return \{\
        downloadedFile: '',\
        downloadedPath: '',\
        downloadError: apiResult.downloadError || 'No visible download button/link found, including top-right coordinate fallback',\
      \};\
    \}\
\
    let download = await firstDownloadPromise;\
\
    if (!download) \{\
      await waitForDownloadModal(probePage);\
      await probePage.waitForTimeout(700);\
\
      download = await clickSourceRowByDom(probePage);\
\
      if (!download) \{\
        let clickedRow = false;\
        const rowClickPoints = await getDownloadModalRowClickPoints(probePage);\
\
        for (let attempt = 1; attempt <= rowClickPoints.length; attempt++) \{\
          const point = rowClickPoints[attempt - 1];\
\
          console.log(\
            `      Trying Downloads modal row click $\{attempt\}/$\{rowClickPoints.length\}: ` +\
            `$\{point.label\} at $\{Math.round(point.x)\}, $\{Math.round(point.y)\}`\
          );\
\
          const modalDownloadPromise = probePage\
            .waitForEvent('download', \{ timeout: 2500 \})\
            .catch(() => null);\
\
          try \{\
            await probePage.mouse.move(point.x, point.y);\
            await probePage.waitForTimeout(100);\
            await probePage.mouse.click(point.x, point.y);\
          \} catch \{\
            continue;\
          \}\
\
...
\
        if (!clickedRow) \{\
          return \{\
            downloadedFile: '',\
            downloadedPath: '',\
            downloadError: apiResult.downloadError || 'Downloads modal opened, but could not click the grey downloadable file row',\
          \};\
        \}\
      \}\
\
...
\
        for (const networkUrl of uniqueNetworkUrls) \{\
          if (!/\\.(mp4|mov|m4v|webm)(\\?|$)|flavorasset|serveFlavor|getUrl|getDownloadUrl/i.test(networkUrl)) \{\
            continue;\
          \}\
\
...
\
        return \{\
          ...
          downloadError:\
            ...
            'Clicked multiple points across the Downloads modal grey row, but no browser download started.',\
        \};\
      \}\
    \}\
\
    const saved = await savePlaywrightDownload(download, activityDir, baseName);\
\
    if (saved.downloadError) \{\
      return \{\
        downloadedFile: '',\
        downloadedPath: '',\
        downloadError: saved.downloadError,\
      \};\
    \}\
\
    return \{\
...
\
  ...
  const courseDir = ensureCourseDirPath(path.join(CONFIG.outputRootDir, courseFolderName));\
\
...
\
      if (earlyDownload.downloaded) \{\
        ...
        \});\
\
        console.log(`  [$\{activityFolderFromPage(`direct-download-$\{processed\}`, url)\}] Downloaded: $\{earlyDownload.downloadedPath\}`);\
        continue;\
      \}\
\
...
\
      ...
      const activityFolder = activityFolderFromPage(pageTitle, page.url());\
...
\
        if (downloadResult.downloadedFile) \{\
          console.log(`  [$\{activityFolder\}] Downloaded: $\{downloadResult.downloadedPath\}`);\
        \}\
      \}\
\
...
\
        if (isKalturaIframeUrl(iframe.url)) \{\
          youtubeUrl = await resolveYoutubeForIframe(context, iframe.url);\
\
          if (youtubeUrl) \{\
            recordedUrl = iframe.url;\
          \} else \{\
            const skinnedIframeUrl = withPlayerSkin(iframe.url);\
            recordedUrl = skinnedIframeUrl;\
\
...
\
        if (youtubeUrl) \{\
          console.log(`      [$\{activityFolder\}] YouTube: $\{youtubeUrl\}`);\
        \}\
\
        if (downloadedFile) \{\
          console.log(`      [$\{activityFolder\}] Downloaded: $\{downloadedPath || downloadedFile\}`);\
        \} else if (downloadError && !IGNORABLE_DOWNLOAD_ERRORS.has(downloadError)) \{\
          console.log(`      [$\{activityFolder\}] Download skipped/failed: $\{downloadError\}`);\
        \}\
      \}\
\
...
\
  const summary = \{\
    ...
    videoDownloadErrors: results.filter(r => r.type === 'iframe' && !r.youtubeUrl && !r.downloadedFile && r.downloadError).length,\
    csvPath,\
    jsonPath,\
  \};\
\
